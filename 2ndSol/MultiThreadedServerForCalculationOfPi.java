...
public class MultiThreadedServerForCalculationOfPi {
	...
	public static void main(String args[]) throws IOException {
		...
		while (j<=clients) {
			j ++;
			//lists to save the start and end of each client and then send it to all of them.
			start.add(Start); 
			end.add(temp);
			if (j == clients)
				temp = temp + pieces + remainder; //last client will get the remainder as well.
			else
				temp = temp + pieces;
			Start += pieces;
		}
		// waiting for all clients to connect and then create and send the socket(client's) start end and step 
		// in order to start compute each of them.
		for (j = 0; j<clients; j++) {
		...
		//wait all threads to finish and then add the returned value to pi.
		for (j = 0;j<clients;j++) {
